...
static void avl_update(AVLNode *node) {
	// curr(1) + max(left,right) for both depth and cnt 
    node->depth = 1 + max(avl_depth(node->left), avl_depth(node->right)); 
    node->cnt = 1 + avl_cnt(node->left) + avl_cnt(node->right);
}

// rotating left for a node 
static AVLNode *rot_left(AVLNode *node) {
	AVLNode *new_node = node->right; // get the right-child of current node
	// check it is a left-child 
	if(new_node->left) {
		new_node->left->parent = node; // make it parent 
	}
...
//        from pointer serves as a reference to the child node (either left or right) of the parent node that is being adjusted.
        AVLNode **from = NULL;
        
        // check if the cur node has a parent
        // current node is a left or right child of its parent.
        if (node->parent) {
            from = (node->parent->left == node ? &node->parent->left : &node->parent->right);
        }
        
        if (l == r + 2) {
            node = avl_fix_left(node); // fix left-sub-tree depth 
        } else if (l + 2 == r) {
            node = avl_fix_right(node); // fix right-sub-tree depth 
        }
        
        // curr node does not have a parent (root)
        if (!from) {
            return node; // successfully balanced 
        }
        
        *from = node; // to update the parent's child  for 
        node = node->parent; // move up the tree for parent-node 
    }
}
